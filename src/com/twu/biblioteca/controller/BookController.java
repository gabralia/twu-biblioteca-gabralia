...
public class BookController {
    ...
    public void checkoutBook(String id){
        Book checkoutBook = bookList.stream().filter(book -> book.getId().equals(id))
                .filter(Book::getAvailable).findFirst().orElse(null);
    ...
    public void returnBook(String id){
        Book checkoutBook = bookList.stream().filter(book -> book.getId().equals(id))
                .filter(book -> !book.getAvailable()).findFirst().orElse(null);
        if(checkoutBook != null) {
            checkoutBook.setAvailable(true);
